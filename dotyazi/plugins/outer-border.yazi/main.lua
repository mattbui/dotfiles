...
function M:setup(opts)
  ...
  local function title_widget(area)
    ...
    if path_max > 0 then
      text = prefix .. ui.truncate(path, { max = path_max, rtl = true }) .. suffix
    else
  ...
  Root.build = function(self, ...)
    ...
    local frame = {
      ui.Border(ui.Edge.ALL):area(self._area):type(border_type):style(th.outer_border.border),
    }
